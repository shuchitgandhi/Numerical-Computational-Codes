...
void MatrixEquationSolver2(double **, int, int);
...
int main()
{
	...
	fstream fin;
	fin.open("MatrixEquationSolver2.dat", ios::in);
	...
	for(i=0;i<rows;i++)
			for(j=0;j<columns;j++)
				fin>>A[i][j];
	MatrixEquationSolver2(A, rows, columns);
	/* Display all answers */
	for(i=0;i<rows;i++)
...
void MatrixEquationSolver2(double **A, int rows, int columns)
{
	int i, j, k;
	double first;
	for(i=0; i<rows;i++)
	{
		...
		for(j=i;j<columns;j++)
			A[i][j] /= first;
		/* Make all elements above and below diagonal element to be 0 */
		for(j=0;j<rows;j++)
		{
			if(j==i)
				continue;
